...
public class BusinessToolService {
    ...
    public String addBusinessUser(String username, String role, boolean needCoupon) {
        ...
        // ================= 模拟系统故障/参数校验失败 =================
        // 假设大模型没有传角色(role 为空或者 "null" 字符串)，我们直接抛异常！
        if (role == null || role.isBlank() || role.equalsIgnoreCase("null")) {
            throw new IllegalArgumentException("业务校验失败：必须指定用户的角色(role)，如 'admin', 'user' 或 'student'！");
        }
        // ==========================================================
        ...
        String mockUserId = "UID-" + System.currentTimeMillis();
        if (needCoupon) {
            return String.format("成功创建用户 [%s], ID为 [%s]。并已触发异步发券逻辑。", username, mockUserId);
        }
        return String.format("成功创建用户 [%s], ID为 [%s]。", username, mockUserId);
    }
}